...
class ProSEOMasterLinkChecker
{
    ...
    protected function getUrlStatus($url)
    {
        ...
        // SSL verification - try with SSL first, fallback without if fails
        $useSSL = true;
        if (file_exists(_PS_TOOL_DIR_ . 'cacert.pem')) {
        ...
        // SSL certificate errors (60, 77, 35) - retry without verification
        if ($result['code'] === 0 && in_array($curlErrno, array(60, 77, 35, 51))) {
            curl_close($ch);
            $ch = curl_init();
            ...
            if ($result['code'] === 0) {
                ...
                $result['ssl_warning'] = 'SSL verification bypassed';
            }
